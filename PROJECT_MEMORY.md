...
- Thiết kế Landing Page: Tránh các timeline dọc đơn điệu chiếm quá ít không gian ngang trên màn rộng; ưu tiên chuyển đổi thành grid-cards (như `pipeline-card`) để tận dụng diện tích màn hình. Thay thế các Interactive Simulator quá chật chội/phức tạp bằng các Browser Mockup thủy tinh hiển thị ảnh chụp màn hình Workspace thực tế để tăng độ chuyên nghiệp.
