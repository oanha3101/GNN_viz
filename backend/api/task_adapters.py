...
def build_label_map(class_names):
    return {str(value): int(index) for index, value in enumerate(class_names or [])}


def stratified_node_split(labels, train_ratio=0.6, val_ratio=0.2, seed=42):
    ...
    for index, label in enumerate(labels):
        labels_to_indices.setdefault(int(label), []).append(index)
    ...
    if not test_indices:
        test_indices = list(val_indices) if val_indices else list(train_indices)
...
class NodeClassificationAdapter(BaseTaskAdapter):
    ...
    def process(self, df_nodes, df_edges, df_graphs, mapping, node_mapper, num_nodes):
        ...
        train_mask, val_mask, test_mask = stratified_node_split(common['labels'])
        for i, node_entry in enumerate(nodes_json):
            node_entry["inTrainSet"] = bool(train_mask[i].item())

        def mask_class_counts(mask):
            counts = [0] * int(common['num_classes'])
            for index, enabled in enumerate(mask.cpu().tolist()):
                if enabled and 0 <= index < len(common['labels']):
                    label = int(common['labels'][index])
                    if 0 <= label < len(counts):
                        counts[label] += 1
            return counts

        split_class_counts = {
            "train": mask_class_counts(train_mask),
            "val": mask_class_counts(val_mask),
            "test": mask_class_counts(test_mask),
        }
...
class GraphClassificationAdapter(BaseTaskAdapter):
    ...
    def process(self, df_nodes, df_edges, df_graphs, mapping, node_mapper, num_nodes):
        ...
        if df_graphs is not None and len(df_graphs) > 0 and m.graph_label:
            ...
            label_map = {str(value): int(index) for index, value in enumerate(label_encoder.classes_)}
        else:
            num_graph_classes = 0
            class_names = []
            label_map = {}
        ...
        for gid, group in df_nodes.groupby(graph_id_col):
            ...
            pyg_data.source_graph_id = str(gid)
            pyg_data.source_index = len(pyg_graphs)
            pyg_data.class_names = class_names
            pyg_data.label_map = label_map
            pyg_graphs.append(pyg_data)

            source_index = len(graphs_json)
            graphs_json.append({
                "id": int(source_index),
                "originalGraphId": int(source_index),
                "sourceIndex": int(source_index),
                "sourceGraphId": str(gid),
...
class CommunityDetectionAdapter(BaseTaskAdapter):
    ...
    def process(self, df_nodes, df_edges, df_graphs, mapping, node_mapper, num_nodes):
        ...
        if m.community_label and m.community_label in df_nodes.columns:
            ...
            label_map = {str(c): int(i) for i, c in enumerate(encoder.classes_)}
        ...
        graph_json = {
            ...
            "hasCommunityGT": community_gt is not None,
            "communityClassNames": class_names,
            "communityLabelMap": label_map,
        ...
        y_tensor = torch.tensor(
            community_gt if community_gt is not None else [0] * num_nodes, dtype=torch.long
        )
