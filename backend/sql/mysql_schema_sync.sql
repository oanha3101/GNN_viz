...
CALL sp_add_index_if_missing('users', 'uq_users_email', 'CREATE UNIQUE INDEX uq_users_email ON users (email)');
CALL sp_add_index_if_missing('users', 'uq_users_username', 'CREATE UNIQUE INDEX uq_users_username ON users (username)');
...
CALL sp_add_index_if_missing('projects', 'idx_projects_owner_id', 'CREATE INDEX idx_projects_owner_id ON projects (owner_id)');
...
CALL sp_add_index_if_missing('datasets', 'uq_datasets_slug', 'CREATE UNIQUE INDEX uq_datasets_slug ON datasets (slug)');
CALL sp_add_index_if_missing('datasets', 'idx_datasets_name', 'CREATE INDEX idx_datasets_name ON datasets (name)');
CALL sp_add_index_if_missing('datasets', 'idx_datasets_owner_id', 'CREATE INDEX idx_datasets_owner_id ON datasets (owner_id)');
CALL sp_add_index_if_missing('datasets', 'idx_datasets_current_version_id', 'CREATE INDEX idx_datasets_current_version_id ON datasets (current_version_id)');
...
CALL sp_add_index_if_missing('dataset_versions', 'uq_dataset_version', 'CREATE UNIQUE INDEX uq_dataset_version ON dataset_versions (dataset_id, version)');
CALL sp_add_index_if_missing('dataset_versions', 'idx_dataset_versions_dataset_id', 'CREATE INDEX idx_dataset_versions_dataset_id ON dataset_versions (dataset_id)');
CALL sp_add_index_if_missing('dataset_versions', 'idx_dataset_versions_lifecycle', 'CREATE INDEX idx_dataset_versions_lifecycle ON dataset_versions (lifecycle)');
CALL sp_add_index_if_missing('dataset_versions', 'idx_dataset_versions_created_by', 'CREATE INDEX idx_dataset_versions_created_by ON dataset_versions (created_by)');
CALL sp_add_index_if_missing('dataset_versions', 'idx_dataset_versions_published_by', 'CREATE INDEX idx_dataset_versions_published_by ON dataset_versions (published_by)');
...
CALL sp_add_index_if_missing('experiments', 'idx_experiments_project_id', 'CREATE INDEX idx_experiments_project_id ON experiments (project_id)');
CALL sp_add_index_if_missing('experiments', 'idx_experiments_owner_id', 'CREATE INDEX idx_experiments_owner_id ON experiments (owner_id)');
CALL sp_add_index_if_missing('experiments', 'idx_experiments_dataset_id', 'CREATE INDEX idx_experiments_dataset_id ON experiments (dataset_id)');
CALL sp_add_index_if_missing('experiments', 'idx_experiments_dataset_version_id', 'CREATE INDEX idx_experiments_dataset_version_id ON experiments (dataset_version_id)');
CALL sp_add_index_if_missing('experiments', 'idx_experiments_task_type', 'CREATE INDEX idx_experiments_task_type ON experiments (task_type)');
CALL sp_add_index_if_missing('experiments', 'idx_experiments_model_type', 'CREATE INDEX idx_experiments_model_type ON experiments (model_type)');
CALL sp_add_index_if_missing('experiments', 'idx_experiments_status', 'CREATE INDEX idx_experiments_status ON experiments (status)');
CALL sp_add_index_if_missing('experiments', 'idx_experiments_mongo_run_id', 'CREATE INDEX idx_experiments_mongo_run_id ON experiments (mongo_run_id)');
CALL sp_add_index_if_missing('experiments', 'idx_experiments_mongo_graph_payload_id', 'CREATE INDEX idx_experiments_mongo_graph_payload_id ON experiments (mongo_graph_payload_id)');
CALL sp_add_index_if_missing('experiments', 'idx_experiments_mongo_metrics_id', 'CREATE INDEX idx_experiments_mongo_metrics_id ON experiments (mongo_metrics_id)');
CALL sp_add_index_if_missing('experiments', 'idx_experiments_created_at', 'CREATE INDEX idx_experiments_created_at ON experiments (created_at)');
...
CALL sp_add_index_if_missing('training_sessions', 'idx_training_sessions_user_id', 'CREATE INDEX idx_training_sessions_user_id ON training_sessions (user_id)');
CALL sp_add_index_if_missing('training_sessions', 'idx_training_sessions_project_id', 'CREATE INDEX idx_training_sessions_project_id ON training_sessions (project_id)');
CALL sp_add_index_if_missing('training_sessions', 'idx_training_sessions_experiment_id', 'CREATE INDEX idx_training_sessions_experiment_id ON training_sessions (experiment_id)');
CALL sp_add_index_if_missing('training_sessions', 'idx_training_sessions_dataset_version_id', 'CREATE INDEX idx_training_sessions_dataset_version_id ON training_sessions (dataset_version_id)');
CALL sp_add_index_if_missing('training_sessions', 'idx_training_sessions_status', 'CREATE INDEX idx_training_sessions_status ON training_sessions (status)');
CALL sp_add_index_if_missing('training_sessions', 'idx_training_sessions_mongo_run_id', 'CREATE INDEX idx_training_sessions_mongo_run_id ON training_sessions (mongo_run_id)');
...
CALL sp_add_index_if_missing('session_snapshots', 'uq_session_epoch', 'CREATE UNIQUE INDEX uq_session_epoch ON session_snapshots (session_id, epoch)');
CALL sp_add_index_if_missing('session_snapshots', 'idx_session_snapshots_experiment_id', 'CREATE INDEX idx_session_snapshots_experiment_id ON session_snapshots (experiment_id)');

CALL sp_add_index_if_missing('audit_logs', 'idx_audit_logs_actor_user_id', 'CREATE INDEX idx_audit_logs_actor_user_id ON audit_logs (actor_user_id)');
CALL sp_add_index_if_missing('audit_logs', 'idx_audit_logs_action', 'CREATE INDEX idx_audit_logs_action ON audit_logs (action)');
CALL sp_add_index_if_missing('audit_logs', 'idx_audit_logs_target_type', 'CREATE INDEX idx_audit_logs_target_type ON audit_logs (target_type)');
CALL sp_add_index_if_missing('audit_logs', 'idx_audit_logs_created_at', 'CREATE INDEX idx_audit_logs_created_at ON audit_logs (created_at)');

CALL sp_add_fk_if_missing('fk_projects_owner_id', 'ALTER TABLE projects ADD CONSTRAINT fk_projects_owner_id FOREIGN KEY (owner_id) REFERENCES users(id)');
CALL sp_add_fk_if_missing('fk_datasets_owner_id', 'ALTER TABLE datasets ADD CONSTRAINT fk_datasets_owner_id FOREIGN KEY (owner_id) REFERENCES users(id)');
CALL sp_add_fk_if_missing('fk_dataset_versions_dataset_id', 'ALTER TABLE dataset_versions ADD CONSTRAINT fk_dataset_versions_dataset_id FOREIGN KEY (dataset_id) REFERENCES datasets(id)');
CALL sp_add_fk_if_missing('fk_dataset_versions_created_by', 'ALTER TABLE dataset_versions ADD CONSTRAINT fk_dataset_versions_created_by FOREIGN KEY (created_by) REFERENCES users(id)');
CALL sp_add_fk_if_missing('fk_dataset_versions_published_by', 'ALTER TABLE dataset_versions ADD CONSTRAINT fk_dataset_versions_published_by FOREIGN KEY (published_by) REFERENCES users(id)');
CALL sp_add_fk_if_missing('fk_datasets_current_version_id', 'ALTER TABLE datasets ADD CONSTRAINT fk_datasets_current_version_id FOREIGN KEY (current_version_id) REFERENCES dataset_versions(id)');
CALL sp_add_fk_if_missing('fk_experiments_project_id', 'ALTER TABLE experiments ADD CONSTRAINT fk_experiments_project_id FOREIGN KEY (project_id) REFERENCES projects(id)');
CALL sp_add_fk_if_missing('fk_experiments_owner_id', 'ALTER TABLE experiments ADD CONSTRAINT fk_experiments_owner_id FOREIGN KEY (owner_id) REFERENCES users(id)');
CALL sp_add_fk_if_missing('fk_experiments_dataset_id', 'ALTER TABLE experiments ADD CONSTRAINT fk_experiments_dataset_id FOREIGN KEY (dataset_id) REFERENCES datasets(id)');
CALL sp_add_fk_if_missing('fk_experiments_dataset_version_id', 'ALTER TABLE experiments ADD CONSTRAINT fk_experiments_dataset_version_id FOREIGN KEY (dataset_version_id) REFERENCES dataset_versions(id)');
CALL sp_add_fk_if_missing('fk_training_sessions_user_id', 'ALTER TABLE training_sessions ADD CONSTRAINT fk_training_sessions_user_id FOREIGN KEY (user_id) REFERENCES users(id)');
CALL sp_add_fk_if_missing('fk_training_sessions_project_id', 'ALTER TABLE training_sessions ADD CONSTRAINT fk_training_sessions_project_id FOREIGN KEY (project_id) REFERENCES projects(id)');
CALL sp_add_fk_if_missing('fk_training_sessions_experiment_id', 'ALTER TABLE training_sessions ADD CONSTRAINT fk_training_sessions_experiment_id FOREIGN KEY (experiment_id) REFERENCES experiments(id)');
CALL sp_add_fk_if_missing('fk_training_sessions_dataset_version_id', 'ALTER TABLE training_sessions ADD CONSTRAINT fk_training_sessions_dataset_version_id FOREIGN KEY (dataset_version_id) REFERENCES dataset_versions(id)');
CALL sp_add_fk_if_missing('fk_session_snapshots_session_id', 'ALTER TABLE session_snapshots ADD CONSTRAINT fk_session_snapshots_session_id FOREIGN KEY (session_id) REFERENCES training_sessions(id)');
CALL sp_add_fk_if_missing('fk_session_snapshots_experiment_id', 'ALTER TABLE session_snapshots ADD CONSTRAINT fk_session_snapshots_experiment_id FOREIGN KEY (experiment_id) REFERENCES experiments(id)');
CALL sp_add_fk_if_missing('fk_audit_logs_actor_user_id', 'ALTER TABLE audit_logs ADD CONSTRAINT fk_audit_logs_actor_user_id FOREIGN KEY (actor_user_id) REFERENCES users(id)');
