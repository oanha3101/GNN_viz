...
def test_task1_streams_each_configured_epoch_without_default_early_stop():
    data = _make_node_data()
    model = GraphSAGEModel(in_channels=4, hidden_channels=8, out_channels=2, num_layers=3, dropout=0.5)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.01)
    websocket = DummyWebSocket()
    ...
    assert [snapshot["epoch"] for snapshot in snapshots] == [0, 1, 2, 3]
    assert snapshots[0]["task1_boundary_patience"] == 0
    assert snapshots[-1]["epochs_target"] == 4
    assert snapshots[-1]["epochs_completed"] == 4
