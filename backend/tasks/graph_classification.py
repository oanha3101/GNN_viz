...
def normalize_graph_labels(graphs):
    """Remap graph labels to contiguous class indices for CrossEntropy/UI parity."""
    labels = []
    for graph in graphs:
    ...
    normalized = [label_to_class[label] for label in labels]

    for graph, class_id in zip(graphs, normalized):
        graph.y = torch.tensor([class_id], dtype=torch.long)

    return normalized, {str(label): class_id for label, class_id in label_to_class.items()}


def build_split_class_counts(indices, labels, num_classes):
...
def get_graph_collection_metadata(graphs):
    first = graphs[0] if graphs else None
    class_names = getattr(first, 'class_names', None)
    label_map = getattr(first, 'label_map', None)
    return {
        'class_names': [str(value) for value in class_names] if isinstance(class_names, (list, tuple)) else None,
        'label_map': dict(label_map) if isinstance(label_map, dict) else None,
    }
...
async def run_graph_classification(config, websocket, stop_flag, custom_graphs=None, snapshot_hook=None):
    ...
    if custom_graphs and len(custom_graphs) > 0:
        ...
        for i, g in enumerate(pyg_graphs):
            ...
            graphs_json.append({
                ...
                'originalGraphId': i,
                'sourceIndex': i,
                **({'sourceGraphId': str(getattr(g, 'source_graph_id'))} if hasattr(g, 'source_graph_id') else {}),
        ...
        graphs_json = [{
            'id': g['id'],
            'originalGraphId': g['id'],
            'sourceIndex': g['id'],
            'groundTruth': ground_truth[index],
            'nodes': g['nodes'],
            'links': g['links'],
            'numNodes': g['numNodes'],
            'numEdges': g['numEdges'],
    ...
    split_class_counts = {
        'train': build_split_class_counts(train_indices, ground_truth, num_classes),
        'val': build_split_class_counts(val_indices, ground_truth, num_classes),
        'test': build_split_class_counts(test_indices, ground_truth, num_classes),
    }
    ...
    for epoch in range(epochs):
        ...
        snapshot = {
            ...
            'early_stopped': bool(will_early_stop),
            'stop_reason': 'early_stop' if will_early_stop else None,
            'label_map': label_map,
    ...
    with torch.no_grad():
        try:
            ...
            final_summary = {
                'model_type': model_type,
                'epochs_target': int(epochs),
                'epochs_completed': int(len(epoch_snapshots)),
                'early_stopped': bool(epoch_snapshots[-1].get('early_stopped')) if epoch_snapshots else False,
                'stop_reason': epoch_snapshots[-1].get('stop_reason') if epoch_snapshots else None,
                'label_map': label_map,
                'best_epoch': int(best_epoch),
                'best_val_epoch': int(best_epoch),
