...
def safe_masked_cross_entropy(logits, target, mask):
    if mask is None or not bool(mask.any()):
        fallback_mask = torch.ones(target.shape[0], dtype=torch.bool, device=target.device)
        return F.cross_entropy(logits[fallback_mask], target[fallback_mask])
    return F.cross_entropy(logits[mask], target[mask])


def safe_masked_accuracy(pred, target, mask):
    if mask is None or not bool(mask.any()):
...
async def run_node_classification(config, data, model, optimizer, websocket, stop_flag, snapshot_hook=None):
    ...
    boundary_patience = int(config.get('task1_boundary_patience', 0))
    snapshot_stride = max(1, int(config.get('task1_snapshot_stride', 1)))
    ...
    for epoch in range(epochs):
        ...
        loss = safe_masked_cross_entropy(out, data.y, data.train_mask)
        loss.backward()
        optimizer.step()
        ...
        with torch.no_grad():
            ...
            val_loss = safe_masked_cross_entropy(out_eval, data.y, data.val_mask)
            pred = out_eval.argmax(dim=1)
            val_acc = safe_masked_accuracy(pred, data.y, data.val_mask)
            train_acc = safe_masked_accuracy(pred, data.y, data.train_mask)
        ...
        if epoch % snapshot_stride == 0 or epoch == epochs - 1 or improved_this_epoch:
            # ── PCA Reduction ───────────────────────────────────────────────────
            try:
            ...
            if is_sage:
                try:
                    sampled_edge_index = train_edge_index.detach().cpu()
                    sampling_edges = []
                    sampled_neighbors = {}
                    max_edges = min(int(sampled_edge_index.shape[1]), 2000)
                    for edge_pos in range(max_edges):
                        src = int(sampled_edge_index[0, edge_pos])
                        tgt = int(sampled_edge_index[1, edge_pos])
                        if src == tgt:
                            continue
                        sampling_edges.append({'source': src, 'target': tgt})
                        sampled_neighbors.setdefault(str(src), []).append(tgt)
                        sampled_neighbors.setdefault(str(tgt), []).append(src)
                except Exception as e:
                    logger.warning("SAGE sampling payload failed: %s", e)
                    sampling_edges = None
                    sampled_neighbors = None
