...
def _graph_json_from_graph_list(graphs: List[Any]) -> dict:
    ...
    label_to_class = {label: idx for idx, label in enumerate(sorted(set(raw_labels)))}
    ground_truth = [label_to_class[label] for label in raw_labels]
    ...
    for graph_index, graph in enumerate(graphs):
        ...
        payload_graphs.append(
            {
                "id": graph_index,
                "originalGraphId": graph_index,
                "sourceIndex": graph_index,
                "groundTruth": ground_truth[graph_index],
    ...
    return {
        ...
        "labelMap": {str(label): class_id for label, class_id in label_to_class.items()},
    }
