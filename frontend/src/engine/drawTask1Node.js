...
export function drawTask1Node(node, ctx, globalScale, config) {
  ...
  const epochDelta = node.epochDelta || null
  ...
  if (!largeGraph && node.isBestSoFar) {
    ctx.beginPath()
    ctx.arc(drawX, drawY, size + 4.5, 0, 2 * Math.PI)
    ctx.strokeStyle = 'rgba(250, 204, 21, 0.32)'
    ctx.lineWidth = 1.1 / globalScale
    ctx.stroke()
  }

  // Main node body — solid color, no inner decoration
  ctx.beginPath()
  ...
  // Node ID label — always centered inside node, white text, no black outline
