...
describe('validateSnapshot', () => {
  it('validates Task 1 snapshot with all required fields', () => {
    const snap = {
      ...
      node_correctness: [1, 0],
      majority_ratio: [0.8, 0.3],
  ...
  it('detects missing fields in Task 1 snapshot', () => {
    ...
    expect(missingFields).toContain('model_type');
    expect(missingFields).toContain('boundary_accuracy');
  });
  ...
  it('requires Task 3 epoch/model metadata to avoid frame drift', () => {
    const snap = {
      epoch: 5,
      edge_scores: [0.9, 0.1],
      train_loss: 0.3,
    ...
    const { valid, missingFields } = validateSnapshot(3, snap);
    expect(valid).toBe(false);
    expect(missingFields).toEqual(['epochs_target', 'epochs_completed', 'model_type']);
  });
  ...
  it('requires Task 4 epoch/model metadata to avoid snapshot-frame drift', () => {
    const snap = {
      epoch: 10,
      node_predictions: [0, 1, 0],
      modularity_q: 0.5,
    ...
    const { valid, missingFields } = validateSnapshot(4, snap);
    expect(valid).toBe(false);
  ...
  it('parses a realistic Task 1 epoch_snapshot from BE', () => {
    ...
    const beMessage = {
      ...
      payload: {
        ...
        majority_ratio: [0.75, 0.6, 0.5, 1.0, 0.67],
        neighbor_majority: [
