    ...
    wsRef.current.onmessage = async (event) => {
      ...
      if (msg.type === 'graph_data') {
        ...
        if (d.graphs) {
          setTaskData({
            graphs: d.graphs,
            ...(d.classNames ? { classNames: d.classNames } : {}),
            ...(d.labelMap ? { labelMap: d.labelMap } : {}),
          })
        }
        ...
        if (d.communityClassNames) {
          setClassNames(d.communityClassNames)
        }
        if (d.numCommunities) {
        ...
        const safeBestEpoch = Number.isFinite(bestEpoch)
          ? (bestSnapshotIndex >= 0 ? bestSnapshotIndex : Math.max(0, maxEpoch))
          : maxEpoch
