...
describe('LabShell run management navigation', () => {
  ...
  it('opens the structure capture route from the header instead of overlaying the topology area', async () => {
    renderLab()
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Mở chế độ chụp' }))

    await waitFor(() => {
      expect(screen.getByText('analysis-page')).toBeInTheDocument()
      expect(screen.getByTestId('location')).toHaveTextContent('/app/lab/analysis/structure')
    })
  })
