...
function ResizableWorkspace({ rightPanelOpen, leftContent, rightContent }) {
  ...
  return (
    <div className="flex-1 flex h-full min-w-0 min-h-0 overflow-hidden">
      ...
      {rightPanelOpen && (
        <div
          className="resize-handle resize-handle-horizontal shrink-0 h-full select-none"
...
function VerticalResizable({ topContent, bottomContent }) {
  ...
  return (
    <div ref={containerRef} className="h-full flex flex-col overflow-hidden">
      ...
      <div
        className="resize-handle resize-handle-vertical shrink-0 w-full select-none"
        onPointerDown={onPointerDown}
        onPointerMove={onPointerMove}
        onPointerUp={onPointerUp}
        onPointerCancel={onPointerUp}
...
function LabShell() {
  ...
  useEffect(() => {
    const handler = () => {
      ...
      const taskTrainingDefaults = buildTaskTrainingDefaults(gnnState.selectedTask, gnnState.selectedModel)
      ...
      window.dispatchEvent(new CustomEvent('gnn:start-training', {
        detail: {
          ...
          epochs: hp.epochs,
          lr: taskTrainingDefaults.lr ?? hp.lr,
          hidden: taskTrainingDefaults.hidden ?? hp.hidden,
          dropout: taskTrainingDefaults.dropout ?? hp.dropout,
          heads: taskTrainingDefaults.heads ?? hp.heads,
          ...
          {(() => {
            ...
            return (
              <div className="flex items-center gap-3">
                <div className="lab-shell-logo-mark w-8 h-8 rounded-md flex items-center justify-center shadow-md ring-1 ring-line-default">
                  <Network size={18} className="lab-shell-logo-icon" />
                </div>
                <div>
                  <h1 className="lab-shell-logo-wordmark text-sm font-black tracking-[0.15em]">GNN-INSIGHT</h1>
                  <div className="lab-shell-logo-sub text-[9px] font-bold uppercase tracking-widest flex items-center gap-1.5">
