...
describe('task 2 collection helpers', () => {
  ...
  it('uses prediction and ground truth when emitted graph_correct drifts', () => {
    const driftSnapshot = {
      ...snapshot,
      graph_predictions: [0, 1, 1, 2],
      graph_correct: [0, 0, 1, 1],
    }

    const descriptors = buildTask2GraphDescriptors({ snapshot: driftSnapshot, graphs, classNames: ['A', 'B', 'C'] })

    expect(descriptors[0].correct).toBe(1)
    expect(descriptors[0].correctnessMismatch).toBe(true)
  })
