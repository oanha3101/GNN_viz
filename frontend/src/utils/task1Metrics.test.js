...
describe('computeAttentionFocus', () => {
  ...
  it('falls back to legacy attention_weights when attention_edges are absent', () => {
    const focus = computeAttentionFocus({ attention_weights: [0.7, 0.2, 0.1] })
    expect(focus.edgeCount).toBe(3)
    expect(focus.topEdgeShare).toBeCloseTo(0.7)
  })
})
...
describe('buildTask1ModelSignature', () => {
  ...
  it('builds a GraphSAGE signature from boundary resilience', () => {
    ...
    expect(signature.primaryValue).toBe('82%')
  })
})
