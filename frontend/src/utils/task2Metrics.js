...
export function buildTask2GraphDescriptors({ snapshot, graphs = [], classNames = [] }) {
  ...
  return normalizedGraphs.map((graph) => {
    ...
    const emittedCorrect = snapshot?.graph_correct?.[sourceIndex]
    const correctnessMismatch = emittedCorrect != null && derivedCorrect != null && emittedCorrect !== derivedCorrect
    const correct = correctnessMismatch ? derivedCorrect : (emittedCorrect ?? derivedCorrect)
