...
/**
 * extractAttentionFocusSeries — pulls attention focus metrics from each
 * snapshot for GAT model visualization. Returns `[{ epoch, focus, top5Share, entropy }]`.
 */
export function extractAttentionFocusSeries(snapshots) {
  if (!Array.isArray(snapshots) || !snapshots.length) return []
  return snapshots.map((s, i) => {
    const attn = computeAttentionFocus(s)
    return {
      epoch: i,
      focus: attn?.focus ?? 0,
      top5Share: attn?.top5Share ?? 0,
      entropy: attn?.entropy ?? 0,
    }
  })
}

/**
 * extractBoundaryResilienceSeries — pulls boundary/interior accuracy and gap
 * from each snapshot for GraphSAGE visualization.
 * Returns `[{ epoch, boundary, interior, gap }]`.
 */
export function extractBoundaryResilienceSeries(snapshots) {
  if (!Array.isArray(snapshots) || !snapshots.length) return []
  return snapshots.map((s, i) => {
    const b = Number.isFinite(s?.boundary_accuracy) ? s.boundary_accuracy : null
    const intr = Number.isFinite(s?.interior_accuracy) ? s.interior_accuracy : null
    return {
      epoch: i,
      boundary: b ?? 0,
      interior: intr ?? 0,
      gap: (intr != null && b != null) ? intr - b : 0,
    }
  })
}

/**
 * extractSamplingCoverageSeries — pulls sampling edge coverage percentage
 * from each snapshot for GraphSAGE model. Returns `[{ epoch, coverage, edgeCount }]`.
 */
export function extractSamplingCoverageSeries(snapshots, totalEdges) {
  if (!Array.isArray(snapshots) || !snapshots.length) return []
  const denom = Math.max(1, totalEdges || 1)
  return snapshots.map((s, i) => {
    const count = Array.isArray(s?.sampling_edges) ? s.sampling_edges.length : 0
    return {
      epoch: i,
...
export function computeAttentionFocus(snapshot) {
  const edgeWeights = (snapshot?.attention_edges || [])
    .map((edge) => Number(edge?.weight || 0))
  const legacyWeights = Array.isArray(snapshot?.attention_weights)
    ? snapshot.attention_weights.map((value) => Number(value || 0))
    : []
  const weights = (edgeWeights.length ? edgeWeights : legacyWeights)
...
export function buildTask1ModelSignature({ snapshot, snapshots, model }) {
  ...
  const computedBoundary = computeBoundaryStats(snapshot, 0.5)
  const boundary = Number.isFinite(snapshot?.boundary_accuracy)
    ? {
      ...computedBoundary,
      boundaryAccuracy: snapshot.boundary_accuracy,
      boundaryCount: Number.isFinite(snapshot?.boundary_count) ? snapshot.boundary_count : computedBoundary.boundaryCount,
      interiorAccuracy: Number.isFinite(snapshot?.interior_accuracy) ? snapshot.interior_accuracy : computedBoundary.interiorAccuracy,
    }
    : computedBoundary
  ...
  if (safeModel === 'SAGE' || safeModel === 'GraphSAGE') {
