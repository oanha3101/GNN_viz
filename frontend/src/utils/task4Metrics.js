...
const TASK4_PHASE_META = {
  baseline: {
    label: 'Baseline chưa train',
    tone: 'amber',
    note: 'Snapshot này là embedding/chia cụm trước bước optimizer đầu tiên.',
  },
  forming: {
    label: 'Đang hình thành',
    tone: 'cyan',
    note: 'Model bắt đầu gom node theo tín hiệu topology và feature.',
  },
  separating: {
    label: 'Đang tách cụm',
    tone: 'violet',
    note: 'Ranh giới cộng đồng đang được làm rõ qua các epoch.',
  },
  stable: {
    label: 'Ổn định',
    tone: 'emerald',
    note: 'Partition đã ít dao động hơn; đọc kèm stability và validation.',
  },
}

export function getTask4PhaseMeta(phase) {
  return TASK4_PHASE_META[phase] || TASK4_PHASE_META.forming
}
...
export function buildTask4QualitySummary(snap) {
  ...
  const gtAgreement = Number.isFinite(snap?.nmi_score) ? clamp01(snap.nmi_score) : null
  const valReconstructionAuc = Number.isFinite(snap?.val_reconstruction_auc) ? clamp01(snap.val_reconstruction_auc) : null

  const balanceScore = 1 - clamp01(largestCommunityRatio)
  const communityBalance = Number.isFinite(snap?.community_balance) ? clamp01(snap.community_balance) : balanceScore
  ...
  const healthScore = clamp01(
    ...
    clamp01((confidence + 1) / 2) * 0.14 +
    conductanceScore * 0.14 +
    bridgeScore * 0.08 +
    communityBalance * 0.08 +
    (valReconstructionAuc ?? 0.5) * 0.06
  )
  ...
  return {
    ...
    healthScore,
    gtAgreement,
    valReconstructionAuc,
    communityBalance,
    collapseRisk: Boolean(snap?.collapse_risk),
    normalizedLoss: snap?.normalized_loss ?? null,
    generalizationGap: snap?.generalization_gap ?? null,
    trainingPhase: snap?.training_phase || null,
    phaseMeta: getTask4PhaseMeta(snap?.training_phase),
    baselineSimilarity: Number.isFinite(snap?.baseline_similarity) ? clamp01(snap.baseline_similarity) : null,
    visualizationConfidence: Number.isFinite(snap?.visualization_confidence) ? clamp01(snap.visualization_confidence) : null,
  }
}
...
export function buildTask4ReasoningPack(snap, snapshots = [], graphData = null, selectedModel = 'GCN') {
  ...
  const phaseMeta = getTask4PhaseMeta(snap?.training_phase)
  ...
  return {
    ...
    phaseMeta,
    phaseNarrative: `${phaseMeta.label}: ${phaseMeta.note}`,
    modelStability,
    migrationRatio,
  }
}
...
export function buildCommunityAnchors(count = 1, width = 800, height = 600) {
  ...
  if (template) {
    return template.map(({ x, y }) => ({ x: x * radiusX, y: y * radiusY }))
  }
...
// Catmull-Rom spline interpolation for smooth island coastlines
...
export function smoothHull(points, segments = 6) {
  const n = points.length
  if (n < 3) return points
  const closed = [...points, points[0], points[1], points[2]] // wrap for Catmull-Rom (need 3 extra for p3 access)
  const smoothed = []
  for (let i = 0; i < n; i++) {
    const p0 = closed[i]
    const p1 = closed[i + 1]
    ...
    for (let t = 0; t < segments; t++) {
      const f = t / segments
      smoothed.push([
        catmullRom(p0[0], p1[0], p2[0], p3[0], f),
        catmullRom(p0[1], p1[1], p2[1], p3[1], f),
      ])
    }
  }
  return smoothed
...
export function buildCoreCommunityEnvelopes(nodes = [], preds = [], snap = {}, options = {}) {
  ...
  return [...byCommunity.entries()].map(([cid, points]) => {
    ...
    // Use ALL points — no bridge filtering, no distance cutoff
    if (points.length < 3) {
      const radius = Math.max(34, ...points.map((point) => Math.hypot(point.x - centroid.x, point.y - centroid.y) + padding))
      return { cid: Number(cid), type: 'circle', center: centroid, radius }
    }
    const hull = points.map((point) => [point.x, point.y])
...
export function buildTask4PredictionToGroundTruthMap(predictions = [], groundTruth = []) {
  const counts = new Map()
  for (let i = 0; i < predictions.length; i += 1) {
    const pred = predictions[i]
    const gt = groundTruth?.[i]
    if (pred == null || gt == null) continue
    if (!counts.has(pred)) counts.set(pred, new Map())
    const row = counts.get(pred)
    row.set(gt, (row.get(gt) || 0) + 1)
  }

  const mapping = new Map()
  for (const [pred, row] of counts.entries()) {
    let bestGt = null
    let bestCount = -1
    for (const [gt, count] of row.entries()) {
      if (count > bestCount) {
        bestGt = gt
        bestCount = count
      }
    }
    if (bestGt != null) mapping.set(pred, bestGt)
  }
  return mapping
}

export function getTask4OverlayMode(modelName) {
  // Overlays are OFF by default — clean community colors
  // Only activate when user explicitly selects a model
...
export function buildTask4MotionProfile({
  ...
  return {
    chargeStrength: large ? -48 : dense ? -62 : -75,
    linkDistance: large ? 24 : dense ? 30 : 38,
    centerStrength: hasFocus ? 0.08 : 0.06,
    communityStrength: large ? 0.07 : dense ? 0.09 : 0.14 + activeMigration * 0.04,
...
export function buildTask4LayoutIntensity({
  progress = 0,
  modularity = 0,
  selectedCommunityId = null,
} = {}) {
  const safeProgress = clamp01(progress)
  const safeModularity = clamp01(modularity)
  const progressStructure = smoothstep(0.02, 0.92, safeProgress)
  const qualityLift = safeModularity * 0.2
  const structure = clamp01(progressStructure * 0.85 + qualityLift)
  const force = 0.18 + structure * 0.82
  const crossEdgeSpread = 1 + structure * 1.8
  const hullOpacity = 0.24 + smoothstep(0.08, 0.72, structure) * 0.76
  const labelOpacity = smoothstep(0.22, 0.78, structure)
  const migrationOpacity = smoothstep(0.34, 0.84, structure)
  const anchorBlend = smoothstep(0.12, 0.7, structure)
  const hasFocus = selectedCommunityId != null
  ...
  return {
    ...
    migrationOpacity,
    anchorBlend,
    selectedDimOpacity: hasFocus ? 0.22 + structure * 0.18 : 1,
    status: structure < 0.35 ? 'forming' : structure < 0.72 ? 'separating' : 'stable',
  }
}

export function buildTask4NodeProfile({ snap, prevSnap = null, nodeId, graphData, communityGroundTruth = null, classNames = null }) {
  ...
  const groundTruthIdx = node?.community ?? node?.communityGT ?? communityGroundTruth?.[nodeId] ?? null
  const gtByIndex = preds.map((_, idx) => {
    const graphNode = graphData?.nodes?.find((item) => item.id === idx)
    return graphNode?.community ?? graphNode?.communityGT ?? communityGroundTruth?.[idx] ?? null
  })
  const predToGroundTruth = buildTask4PredictionToGroundTruthMap(preds, gtByIndex)
  const alignedGroundTruth = community != null ? predToGroundTruth.get(community) : null
  const groundTruthLabel = classNames && groundTruthIdx != null ? classNames[groundTruthIdx] : null
  ...
  return {
    ...
    community,
    groundTruth: groundTruthIdx,
    groundTruthLabel,
    alignedGroundTruth,
    previousCommunity,
    migrated: previousCommunity != null && community != null && previousCommunity !== community,
    isMismatch: groundTruthIdx != null && alignedGroundTruth != null && alignedGroundTruth !== groundTruthIdx,
    isBridge,
    ...
    sageNoiseMigration: snap.sage_noise_migrations?.[nodeId] ?? false,
...
/**
 * extractTask4DirichletSeries — pulls `dirichlet_energy` from each snapshot
 * for GCN smoothness visualization. Returns `[{ epoch, energy }]`.
 */
export function extractTask4DirichletSeries(snapshots) {
  if (!Array.isArray(snapshots) || !snapshots.length) return []
  return snapshots.map((s, i) => ({
    epoch: i,
    energy: Number.isFinite(s?.dirichlet_energy) ? s.dirichlet_energy : 0,
  }))
}

/**
 * extractTask4AttentionBoundarySeries — pulls `attention_boundary_ratio` from
 * each snapshot for GAT visualization. Returns `[{ epoch, ratio }]`.
 */
export function extractTask4AttentionBoundarySeries(snapshots) {
  if (!Array.isArray(snapshots) || !snapshots.length) return []
  return snapshots.map((s, i) => ({
    epoch: i,
    ratio: Number.isFinite(s?.attention_boundary_ratio) ? s.attention_boundary_ratio : null,
  }))
}

/**
 * extractTask4RobustnessSeries — pulls `sage_robustness` from each snapshot
 * for GraphSAGE visualization. Returns `[{ epoch, robustness }]`.
 */
export function extractTask4RobustnessSeries(snapshots) {
  if (!Array.isArray(snapshots) || !snapshots.length) return []
  return snapshots.map((s, i) => ({
    epoch: i,
    robustness: Number.isFinite(s?.sage_robustness) ? s.sage_robustness : null,
  }))
}
