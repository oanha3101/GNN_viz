...
describe('Task2MetricsPanel', () => {
  ...
  it('updates shared cell state when confusion cell is selected', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Failures|Lỗi/i }))
  ...
  it('shows selected cell scope in failures copy', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Failures|Lỗi/i }))
    expect(screen.getByText(/Hard cases below are scoped to this cell|Các ca khó bên dưới đang được lọc theo ô này/i)).toBeInTheDocument()
  })
  ...
  it('surfaces the Task 2 trust profile in the overview', () => {
    ...
    expect(screen.getByText(/Trust profile|Hồ sơ độ tin cậy/i)).toBeInTheDocument()
    expect(screen.getByText(/High-conf wrong|Sai tự tin cao/i)).toBeInTheDocument()
    expect(screen.getByText('Brier')).toBeInTheDocument()
    expect(screen.getAllByText(/Readout diffuse|Readout loãng/i).length).toBeGreaterThan(0)
  })

  it('shows best epoch suggestion guidance in the overview', () => {
    render(<Task2MetricsPanel />)

    expect(screen.getByText(/Best epoch suggestion|Gợi ý epoch tốt nhất/i)).toBeInTheDocument()
    expect(screen.getByText(/Best Macro F1|Macro F1 tốt nhất/i)).toBeInTheDocument()
    expect(screen.getByText(/Best Balanced Acc|Balanced Acc tốt nhất/i)).toBeInTheDocument()
  })
