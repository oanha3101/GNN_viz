...
import {
  ...
  extractTask4DirichletSeries,
  extractTask4AttentionBoundarySeries,
  extractTask4RobustnessSeries,
...
import { TASK4_PANEL_VI, localizeElementText } from '../../utils/panelVietnamese'
...
export default function Task4MetricsPanel({ forcedTab = null, hideTabControls = false }) {
  ...
  useEffect(() => {
    ...
    const rafId = window.requestAnimationFrame(() => {
      localizeElementText(panelRootRef.current, lang, TASK4_PANEL_VI)
    })
    return () => window.cancelAnimationFrame(rafId)
  }, [lang, activeTab, epochInt, snapshots.length])

  if (!snapshots.length) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-slate-400 p-6 gap-2">
...
function OverviewTab({ copy, snap, snapshots, epochInt, graphData, selectedModel }) {
  ...
  const normalizedLoss = snap?.normalized_loss ?? null
  const valReconAuc = snap?.val_reconstruction_auc ?? null
  const generalizationGap = snap?.generalization_gap ?? null
  const communityBalance = snap?.community_balance ?? null
  const rawLoss = snap?.train_loss ?? null
  const showCompositeLossNotice = rawLoss != null && rawLoss > 1 && (summary.modularity > 0.25 || summary.gtAgreement == null || summary.gtAgreement > 0.45)
  const showRiskNotice = Boolean(snap?.collapse_risk || summary.emptyCommunityCount > 0 || summary.largestCommunityRatio > 0.65 || (generalizationGap != null && generalizationGap < -0.25))
  const phaseMeta = summary.phaseMeta
  const baselineSimilarity = summary.baselineSimilarity
  const visualizationConfidence = summary.visualizationConfidence
  const showBaselineNotice = snap?.training_phase === 'baseline' || (snap?.epoch === 0 && baselineSimilarity != null)
  const showStrongBaselineNotice = showBaselineNotice && baselineSimilarity != null && baselineSimilarity >= 0.45
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col gap-3 overflow-auto">
      ...
      >
        ...
        <StatCell
          label={copy.labels.gtAgreement}
          value={summary.gtAgreement == null ? null : summary.gtAgreement * 100}
          digits={1}
          caption={copy.captions.gtAgreement}
          tone={summary.gtAgreement == null ? 'warn' : summary.gtAgreement > 0.75 ? 'good' : summary.gtAgreement > 0.5 ? 'warn' : 'bad'}
        />
        ...
        <StatCell label="Norm loss" value={normalizedLoss} digits={3} caption="Composite loss scaled for easier reading."
          tone={normalizedLoss == null ? 'warn' : normalizedLoss < 0.55 ? 'good' : normalizedLoss < 0.75 ? 'warn' : 'bad'} />
        <StatCell label="Val recon AUC" value={valReconAuc} digits={3} caption="Structural holdout signal; does not use community labels."
          tone={valReconAuc == null ? 'warn' : valReconAuc > 0.72 ? 'good' : valReconAuc > 0.58 ? 'warn' : 'bad'} />
        <StatCell label="Balance" value={communityBalance == null ? null : communityBalance * 100} digits={1} caption="Higher means less community collapse."
          tone={communityBalance == null ? 'warn' : communityBalance > 0.55 ? 'good' : communityBalance > 0.35 ? 'warn' : 'bad'} />
        <StatCell label="Gen gap" value={generalizationGap} digits={3} caption="Train reconstruction loss minus validation reconstruction loss."
          tone={generalizationGap == null ? 'warn' : generalizationGap < -0.25 ? 'bad' : generalizationGap < -0.1 ? 'warn' : 'good'} />
        <StatCell label="Baseline sim" value={baselineSimilarity} digits={3} caption="Structure agreement measured at the untrained baseline."
          tone={baselineSimilarity == null ? 'warn' : baselineSimilarity > 0.55 ? 'good' : baselineSimilarity > 0.35 ? 'warn' : 'neutral'} />
        <StatCell label="Viz confidence" value={visualizationConfidence == null ? null : visualizationConfidence * 100} digits={1} caption="How reliable this visual partition is as an explanation."
          tone={visualizationConfidence == null ? 'warn' : visualizationConfidence > 0.7 ? 'good' : visualizationConfidence > 0.45 ? 'warn' : 'bad'} />
      ...
      {(showBaselineNotice || showCompositeLossNotice || showRiskNotice) && (
        <div className={`rounded-md border px-4 py-3 text-[12px] leading-relaxed ${
          showRiskNotice
            ? 'border-rose-400/30 bg-rose-400/[0.08] text-rose-100'
            : showBaselineNotice
              ? 'border-cyan-400/30 bg-cyan-400/[0.08] text-cyan-100'
      ...
      <div className="overflow-hidden rounded-md border border-cyan-400/20 bg-cyan-400/[0.04] ">
        <div className="border-b border-line-subtle px-4 py-3">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <SignalBadge label="Phase" value={phaseMeta?.label || snap?.training_phase} tone={phaseMeta?.tone || snap?.training_phase} />
        ...
        <div className="rounded-md border border-line-subtle bg-deep p-3">
          <div className="mb-3 flex items-center justify-between gap-2">
            <div>
              ...
              <div className="mt-1 text-xs text-slate-400">{copy.labels.communityMapSubtitle}</div>
            </div>
            <div className="rounded-sm border border-white/[0.08] bg-white/[0.04] px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-100">
      ...
      <div className="rounded-md border border-line-subtle bg-deep px-1 pt-3">
        ...
        <ResponsiveContainer width="100%" height="90%">
          <AreaChart data={historyData}>
            ...
            <XAxis dataKey="epoch" tick={{ fill: '#cbd5e1', fontSize: 9 }} />
            <YAxis domain={[-0.1, 1]} tick={{ fill: '#cbd5e1', fontSize: 9 }} />
            <Tooltip
...
function BridgesTab({ copy, snap, graphData, onFocus }) {
  ...
  if (bridges.length === 0) {
    return (
      <div className="flex-1 flex items-center justify-center text-slate-400 text-micro">
        {copy.empty.bridges}
      </div>
    )
  }
  return (
    <div className="flex-1 overflow-auto space-y-3">
      <div className="text-nano text-slate-200 uppercase font-bold tracking-ultra">{copy.labels.topBridges} - {copy.descriptions.bridgeSorted}</div>
...
function StabilityTab({ copy, snap, snapshots, epochInt }) {
  ...
  return (
    <div className="flex-1 overflow-auto space-y-3">
      ...
      <div>
        <span className="text-nano text-slate-200 uppercase font-bold tracking-ultra block mb-1">{copy.labels.stabilityPerEpoch}</span>
        <ResponsiveContainer width="100%" height={90}>
          <LineChart data={lineData}>
            <CartesianGrid strokeDasharray="3 3" stroke="var(--c-border)" />
            <XAxis dataKey="epoch" tick={{ fill: '#cbd5e1', fontSize: 8 }} />
            <YAxis domain={[0, 1]} tick={{ fill: '#cbd5e1', fontSize: 8 }} />
            <Tooltip
              contentStyle={{ background: 'var(--c-bg-elev)', border: '1px solid var(--c-border)', color: 'var(--c-fg)', borderRadius: 8, fontSize: 10 }}
              itemStyle={{ color: '#e2e8f0' }}
              labelStyle={{ color: '#cbd5e1' }}
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="text-nano text-slate-200 uppercase font-bold tracking-ultra">{copy.labels.stabilityVelocity} (Î")</span>
      ...
      <div>
        <span className="text-nano text-slate-200 uppercase font-bold tracking-ultra block mb-1.5">{copy.labels.stabilityHeatmap}</span>
...
function DiagnosticsTab({ copy, snap, snapshots, epochInt }) {
  ...
  const lossComponents = snap?.loss_components || null
  const normalizedLoss = snap?.normalized_loss
  const valReconAuc = snap?.val_reconstruction_auc
  const generalizationGap = snap?.generalization_gap
  ...
  return (
    <div className="flex-1 overflow-auto space-y-3">
      ...
      {/* Model-specific signature metrics with sparklines */}
      <ModelSignatureSection
        modelType={modelType}
        dirichlet={dirichlet}
        robustness={robustness}
        attnBoundary={attnBoundary}
        snapshots={snapshots}
        epochInt={epochInt}
        copy={copy}
      />
      ...
      {lossComponents && (
        <div className="rounded-md border border-line-subtle bg-deep p-3">
          ...
          <div className="mt-2 grid gap-2" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))' }}>
            ...
            <MiniMetric label="Pos edge" value={lossComponents.pos_loss} />
            <MiniMetric label="Neg edge" value={lossComponents.neg_loss} />
            <MiniMetric label="Cohesion" value={lossComponents.cohesion_loss} />
            <MiniMetric label="Balance" value={lossComponents.balance_penalty} />
            <MiniMetric label="Val recon AUC" value={valReconAuc} />
            <MiniMetric label="Gen gap" value={generalizationGap} />
          </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="text-nano text-slate-200 uppercase font-bold tracking-ultra">{copy.labels.confidenceHistogramTitle}</span>
            ...
            {hist.map((bin, i) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-0.5" title={`${bin.range[0].toFixed(1)}â€"${bin.range[1].toFixed(1)} Â· ${bin.count}`}>
                  ...
                  />
                  <span className="text-nano text-slate-400 font-mono">{bin.range[0].toFixed(1)}</span>
                </div>
              )
            })}
          </div>
...
function HierarchyPreview({ rows }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        <span className="text-nano text-slate-200 uppercase font-bold tracking-ultra">{TASK4_COPY.labels.hierarchyMerges}</span>
        <span className="text-nano text-slate-200 font-mono">{rows.length} {TASK4_COPY.labels.hierarchyRows}</span>
      </div>
  ...
  return (
    <div className="rounded-md border border-line-subtle bg-deep px-3 py-2.5">
      ...
      <div className="mt-2 h-1.5 bg-white/[0.06] rounded-sm overflow-hidden">
        <div className="h-full transition-all duration-300 rounded-sm" style={{ width: `${barV * 100}%`, backgroundColor: color }} />
      </div>
      {/* Mini sparkline */}
      {history.length > 2 && (
...
function ChartCaption({ text }) {
  if (!text) return null
  return <p className="mt-1 text-[10px] leading-snug text-slate-400">{text}</p>
}
...
function SignalBadge({ label, value, tone = 'neutral' }) {
  ...
  const tones = {
    strong: 'border-emerald-500/30 bg-emerald-500/15 text-emerald-400',
    stable: 'border-emerald-500/30 bg-emerald-500/15 text-emerald-400',
    good: 'border-emerald-500/30 bg-emerald-500/15 text-emerald-400',
    tot: 'border-emerald-500/30 bg-emerald-500/15 text-emerald-400',
    mixed: 'border-slate-400/20 bg-white/[0.06] text-slate-100',
    warn: 'border-amber-500/30 bg-amber-500/15 text-amber-300',
    canh_bao: 'border-amber-500/30 bg-amber-500/15 text-amber-300',
    boundary: 'border-rose-500/30 bg-rose-500/15 text-rose-300',
    leakage: 'border-orange-500/30 bg-orange-500/15 text-orange-300',
    migration: 'border-cyan-500/30 bg-cyan-500/15 text-cyan-300',
    confidence: 'border-fuchsia-500/30 bg-fuchsia-500/15 text-fuchsia-300',
    baseline: 'border-amber-500/30 bg-amber-500/15 text-amber-300',
    forming: 'border-cyan-500/30 bg-cyan-500/15 text-cyan-300',
    separating: 'border-fuchsia-500/30 bg-fuchsia-500/15 text-fuchsia-300',
    ...
    unstable_drop: 'border-rose-500/30 bg-rose-500/15 text-rose-300',
    bad: 'border-rose-500/30 bg-rose-500/15 text-rose-300',
    yeu: 'border-rose-500/30 bg-rose-500/15 text-rose-300',
  }

  return (
    <div className={`rounded-sm border px-2.5 py-1 ${tones[tone] || 'border-white/[0.08] bg-white/[0.04] text-slate-100'}`}>
      <div className="text-[9px] uppercase tracking-[0.2em] text-slate-200">{label}</div>
  ...
  return (
    <div className="min-w-0 rounded-md border border-line-subtle bg-deep p-3">
      <div className="flex items-start justify-between gap-2">
        ...
        <span className="rounded-sm border border-white/[0.08] px-2 py-0.5 text-[10px] font-bold text-slate-100">
          {card.size} {TASK4_COPY.labels.nodesInCommunity.toLowerCase()}
        </span>
      </div>

      <div className="mt-2 text-[11px] leading-relaxed text-slate-300">{card.status}</div>
  ...
  return (
    <div className="rounded-md border border-line-subtle bg-deep p-3">
      ...
      <div className="mt-2 h-1.5 overflow-hidden rounded-sm bg-white/[0.06]">
        <div className="h-full rounded-sm transition-all" style={{ width: `${Math.max(0, Math.min(1, normalized)) * 100}%`, backgroundColor: color }} />
      </div>
      <div className="mt-1.5 text-[10px] leading-relaxed text-slate-400">{note}</div>
    </div>
  )
}
...
function MiniProgress({ label, value, color, right }) {
  return (
    <div>
      <div className="mb-1 flex items-center justify-between gap-2 text-[10px] uppercase tracking-[0.16em] text-slate-100">
        <span>{label}</span>
        <span className="font-mono font-bold text-slate-200">{right}</span>
      </div>
      <div className="h-1.5 overflow-hidden rounded-sm bg-white/[0.06]">
        <div className="h-full rounded-sm" style={{ width: `${Math.max(0, Math.min(1, value)) * 100}%`, backgroundColor: color }} />
      </div>
    </div>
  )
}
...
function IssueStrip({ title, tone = 'neutral', community = null, metricLabel = '', metricValue = '', body = '', fallback = '' }) {
  ...
  return (
    <div className={`rounded-md border p-3  ${toneMap[tone] || toneMap.neutral}`}>
      <div className="text-[10px] uppercase tracking-[0.2em] text-slate-200">{title}</div>
      ...
      <div className="mt-2 text-[12px] leading-relaxed text-slate-200">{resolvedBody || fallback}</div>
    </div>
  )
}
...
function EpochStrip({ summary }) {
  return (
    <div className="rounded-md border border-cyan-400/20 bg-cyan-400/[0.05] p-3 ">
      <div className="text-[10px] uppercase tracking-[0.2em] text-slate-200">{TASK4_COPY.labels.epochChange}</div>
      <div className="mt-2 text-[12px] leading-relaxed text-slate-200">{summary}</div>
    </div>
  )
}
...
function BridgeGroup({ title, items, onFocus, copy }) {
  ...
  return (
    <div className="space-y-1.5">
      <div className="text-[10px] uppercase tracking-[0.18em] text-slate-200">{title}</div>
      {items.map((b) => (
        <button
          key={`${title}-${b.id}`}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="text-xs font-mono font-bold text-white">{b.strength.toFixed(2)}</div>
              </div>
...
function StatCell({ label, value, digits = 0, tone = 'neutral', caption = '' }) {
  const toneStyles = {
    good: { value: 'text-emerald-400', accent: '#34d399', bg: 'rgba(16,185,129,0.06)', border: 'rgba(16,185,129,0.18)' },
    warn: { value: 'text-amber-400', accent: '#fbbf24', bg: 'rgba(245,158,11,0.06)', border: 'rgba(245,158,11,0.18)' },
    bad: { value: 'text-rose-400', accent: '#fb7185', bg: 'rgba(244,63,94,0.06)', border: 'rgba(244,63,94,0.18)' },
    neutral: { value: 'text-white', accent: '#e2e8f0', bg: 'rgba(255,255,255,0.03)', border: 'rgba(255,255,255,0.08)' },
  }[tone]
  const display = Number.isFinite(value) ? (digits > 0 ? value.toFixed(digits) : `${value}`) : '—'
  return (
    <div className="rounded-md border px-3 py-2.5 text-center" style={{ borderColor: toneStyles.border, backgroundColor: toneStyles.bg }}>
      <span className="block text-[9px] uppercase tracking-[0.18em] font-bold text-slate-300">{label}</span>
      <span className={`mt-1.5 block text-[20px] font-black font-mono tabular-nums ${toneStyles.value}`}>{display}</span>
      {caption && <span className="mt-1.5 block text-[10px] leading-snug text-slate-400 normal-case tracking-normal">{caption}</span>}
    </div>
  )
}
...
/* ── Model Signature Section with sparklines ──────────────────────────────── */
function ModelSignatureSection({ modelType, dirichlet, robustness, attnBoundary, snapshots, epochInt, copy }) {
  const dirichletSeries = useMemo(() => extractTask4DirichletSeries(snapshots), [snapshots])
  const attentionSeries = useMemo(() => extractTask4AttentionBoundarySeries(snapshots), [snapshots])
  const robustnessSeries = useMemo(() => extractTask4RobustnessSeries(snapshots), [snapshots])

  if (dirichlet == null && robustness == null && attnBoundary == null) return null

  return (
    <div>
      <span className="text-nano text-slate-200 uppercase font-bold tracking-ultra block mb-1">
      ...
      <div className="grid grid-cols-3 gap-3">
        {dirichlet != null && (
          <GaugeWithSparkline
            label="Dirichlet E"
            value={dirichlet}
            history={dirichletSeries.slice(0, epochInt + 1)}
            dataKey="energy"
            color="#a78bfa"
            epochInt={epochInt}
          />
        ...
        {robustness != null && (
          <GaugeWithSparkline
            label="Robustness"
            value={robustness}
            history={robustnessSeries.slice(0, epochInt + 1)}
            dataKey="robustness"
            color="#22d3ee"
            epochInt={epochInt}
          />
        ...
        {attnBoundary != null && (
          <GaugeWithSparkline
            label="Attn Boundary"
            value={attnBoundary}
            history={attentionSeries.slice(0, epochInt + 1)}
            dataKey="ratio"
            color="#fbbf24"
            epochInt={epochInt}
          />
