...
function OverviewTab({ copy, snap, snapshots, epochInt, paired, heroEdge, roc, pr, summary }) {
  ...
  const modelType = snap?.model_type || 'GCN'
  const deltaValues = Array.isArray(snap?.edge_score_delta) ? snap.edge_score_delta : []
  const meanAbsDelta = deltaValues.length
    ? deltaValues.reduce((sum, value) => sum + Math.abs(value || 0), 0) / deltaValues.length
    : null
  const modelSignal = modelType === 'GAT'
    ? snap?.attention_focus_score
    : modelType === 'SAGE'
      ? snap?.score_stability
      : snap?.smoothness_separation
  ...
  return (
    <div className="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar">
      ...
      >
        ...
        <Metric label={copy.labels.model} value={modelType} tone="slate" />
        <Metric label={copy.labels.bestAuc} value={snap?.best_auc == null ? '-' : snap.best_auc.toFixed(3)} tone="emerald" />
        <Metric label={copy.labels.delta} value={meanAbsDelta == null ? '-' : meanAbsDelta.toFixed(3)} tone="cyan" />
        <Metric
          label={modelType === 'GAT' ? copy.labels.attentionFocus : modelType === 'SAGE' ? copy.labels.scoreStability : copy.labels.smoothnessSeparation}
          value={modelSignal == null ? '-' : Number(modelSignal).toFixed(3)}
          tone={modelType === 'SAGE' ? 'cyan' : modelType === 'GAT' ? 'amber' : 'emerald'}
        />
      </div>
          ...
          <div className="h-40 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={histogram} margin={{ top: 6, right: 10, bottom: 14, left: 0 }}>
                ...
                <Tooltip contentStyle={tooltipStyle} itemStyle={tooltipItemStyle} labelStyle={tooltipLabelStyle} />
                <Bar dataKey="positive" stackId="a" fill="#22c55e" name={copy.labels.positive} />
                <Bar dataKey="negative" stackId="a" fill="#ef4444" name={copy.labels.negative} />
              </BarChart>
            </ResponsiveContainer>
          </div>
