...
const MODEL_ACTIVE_FILL_CLASS = {
  GCN: 'lab-model-fill-gcn',
  GAT: 'lab-model-fill-gat',
  SAGE: 'lab-model-fill-sage',
}
              ...
              return (
                <button
                  ...
                  className={`relative flex-1 rounded-lg py-2 text-[10px] font-black transition-all duration-300 uppercase tracking-wide transform hover:scale-[1.03] active:scale-95 ${
                    isSelected ? 'lab-sidebar-active-fg' : 'text-moonlight hover:text-starlight hover:bg-line-default/30'
                  } ${collapsed ? 'w-full px-0' : 'px-2'}`}
                >
                  {isSelected && (
                    <motion.div
                      layoutId="model-active-bg"
                      className={`absolute inset-0 rounded-lg ${MODEL_ACTIVE_FILL_CLASS[model.id] ?? 'lab-sidebar-active-fill'}`}
                      transition={{ type: 'spring', bounce: 0.2, duration: 0.6 }}
                    />
                  )}
                  <span className={`relative z-10 ${isSelected ? 'lab-sidebar-active-fg' : ''}`}>{model.label}</span>
                </button>
              )
            })}
          </div>
