...
function RunChip({ selected, onClick, children, className = '', ...rest }) {
  return (
    <button
      onClick={onClick}
      {...rest}
      className={`rounded-md border px-2.5 py-1 text-[11px] font-semibold transition-all ${className}`}
    >
      {children}
    </button>
  )
...
const getStatusBadgeClass = (theme, status, isSelected) => {
  if (theme === 'light') {
    const map = {
      completed: 'bg-emerald-50 text-emerald-700 border-emerald-200/50',
      running: 'bg-amber-50 text-amber-700 border-amber-200/50 animate-pulse',
      failed: 'bg-rose-50 text-rose-700 border-rose-200/50',
      stopped: 'bg-slate-50 text-slate-700 border-slate-200/50',
    }
    return `inline-flex items-center gap-1.5 rounded-md border px-2 py-0.5 text-[11px] font-medium transition-colors ${map[status] || 'bg-slate-50 text-slate-700 border-slate-200/50'}`
  } else {
    // Dark mode
    const map = {
    ...
    return `inline-flex items-center gap-1.5 rounded-md border px-2 py-0.5 text-[11px] font-medium transition-colors ${map[status] || 'bg-slate-500/10 text-slate-400 border-slate-500/20'}`
  }
}

const getThemeColorStyles = (theme, isSelected) => {
  if (!isSelected) {
    if (theme === 'light') {
      return {
        cardClass: 'bg-slate-50 hover:bg-slate-100/80 hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:-translate-y-0.5 ring-1 ring-slate-200/50 hover:ring-slate-200 text-slate-700',
        titleClass: 'text-slate-800 group-hover:text-slate-900 font-semibold',
        metaClass: 'text-slate-500',
        badgeBg: 'bg-slate-200/60',
        badgeText: 'text-slate-600',
        metricLabelClass: 'text-slate-500',
        metricValueBg: 'bg-slate-200/50 text-slate-800',
        notesBg: 'bg-slate-100/50 border-slate-200/40 text-slate-600',
        btnReplay: 'border-aurora-blue/20 bg-aurora-blue/10 text-aurora-blue hover:bg-aurora-blue/20',
        btnPdfView: 'border-amethyst/25 bg-amethyst/10 text-amethyst hover:bg-amethyst/20',
        btnPdfDownload: 'border-aurora-green/25 bg-aurora-green/10 text-aurora-green hover:bg-aurora-green/20',
        btnDelete: 'border-aurora-rose/20 bg-aurora-rose/10 text-aurora-rose hover:bg-aurora-rose/20',
        runChipClass: 'border-slate-300 bg-white text-slate-700 hover:text-slate-900 hover:border-slate-400',
        runChipSelectedClass: 'border-aurora-blue/40 bg-aurora-blue/12 text-aurora-blue'
      }
    } else {
      return {
        cardClass: 'bg-white/5 hover:bg-white/10 hover:shadow-lg hover:-translate-y-0.5 ring-1 ring-transparent hover:ring-white/10 text-starlight',
        titleClass: 'text-white-star group-hover:text-white',
        metaClass: 'text-twilight',
        badgeBg: 'bg-white/5',
        badgeText: 'text-moonlight',
        metricLabelClass: 'text-twilight/80',
        metricValueBg: 'bg-white/5 text-white-star',
        notesBg: 'bg-white/5 border-line-subtle text-starlight/90',
        btnReplay: 'border-aurora-blue/20 bg-aurora-blue/10 text-aurora-blue hover:bg-aurora-blue/20',
        btnPdfView: 'border-amethyst/25 bg-amethyst/10 text-amethyst hover:bg-amethyst/20',
        btnPdfDownload: 'border-aurora-green/25 bg-aurora-green/10 text-aurora-green hover:bg-aurora-green/20',
        btnDelete: 'border-aurora-rose/20 bg-aurora-rose/10 text-aurora-rose hover:bg-aurora-rose/20',
        runChipClass: 'border-line-subtle bg-glass text-twilight hover:text-starlight hover:border-line-active',
        runChipSelectedClass: 'border-aurora-blue/40 bg-aurora-blue/12 text-aurora-blue'
      }
    }
  }
  ...
  if (theme === 'light') {
    // Light mode selected: vibrant red background, white text and elements
    return {
      cardClass: 'bg-gradient-to-r from-red-50 to-pink-50/70 shadow-[0_8px_24px_rgba(220,38,38,0.06)] ring-1 ring-red-200 scale-[1.01] text-slate-800',
      titleClass: 'text-red-700 font-bold',
      metaClass: 'text-slate-500',
      badgeBg: 'bg-red-500/10',
      badgeText: 'text-red-700',
      metricLabelClass: 'text-slate-500',
      metricValueBg: 'bg-red-50 text-red-700',
      notesBg: 'bg-red-500/5 border-red-100 text-slate-600',
      btnReplay: 'border-red-200 bg-red-50 text-red-600 hover:bg-red-100',
      btnPdfView: 'border-purple-200 bg-purple-50 text-purple-600 hover:bg-purple-100',
      btnPdfDownload: 'border-emerald-200 bg-emerald-50 text-emerald-600 hover:bg-emerald-100',
      btnDelete: 'border-rose-200 bg-rose-50 text-rose-600 hover:bg-rose-100',
      runChipClass: 'border-slate-300 bg-white text-slate-700 hover:text-slate-900 hover:bg-slate-50',
      runChipSelectedClass: 'border-red-500 bg-red-600 text-white'
    }
  } else {
    // Dark mode selected: vibrant cyan background, dark text and elements
    return {
      cardClass: 'bg-gradient-to-r from-cyan-950/40 to-purple-950/20 shadow-[0_8px_32px_rgba(6,182,212,0.12)] ring-1 ring-cyan-500/40 scale-[1.01] text-starlight',
      titleClass: 'text-cyan-400 font-bold',
      metaClass: 'text-twilight',
      badgeBg: 'bg-cyan-500/10',
      badgeText: 'text-cyan-300',
      metricLabelClass: 'text-twilight/80',
      metricValueBg: 'bg-cyan-500/10 text-cyan-300',
      notesBg: 'bg-purple-500/5 border-purple-500/10 text-starlight/95',
      btnReplay: 'border-cyan-500/30 bg-cyan-500/10 text-cyan-300 hover:bg-cyan-500/20',
      btnPdfView: 'border-purple-500/30 bg-purple-500/10 text-purple-300 hover:bg-purple-500/20',
      btnPdfDownload: 'border-emerald-500/30 bg-emerald-500/10 text-emerald-300 hover:bg-emerald-500/20',
      btnDelete: 'border-rose-500/30 bg-rose-500/10 text-rose-300 hover:bg-rose-500/20',
      runChipClass: 'border-line-subtle bg-glass text-twilight hover:text-starlight hover:border-line-active',
      runChipSelectedClass: 'border-cyan-400/40 bg-cyan-400/12 text-cyan-300'
    }
  }
}
...
export default function ExperimentHub({ isOpen, onClose, variant = 'modal' }) {
  ...
  const shellInnerClass = isPage
    ...
    : 'mx-auto flex h-full max-w-7xl flex-col overflow-hidden rounded-md glass-panel shadow-panel-lg border-line-subtle'
      ...
      >
        ...
        <div className="relative z-10 flex items-center justify-between px-6 pt-6 pb-2">
          ...
          {!isPage ? (
            <button
              onClick={onClose}
              className="rounded-md border border-line-subtle bg-glass p-2.5 text-starlight hover:bg-glass-hover hover:border-line-active transition-colors"
            >
              <X size={16} />
            </button>
          ) : null}
        ...
        <div className="relative z-10 px-6 pb-4 pt-2">
          <div className="flex flex-wrap items-end gap-3 rounded-md bg-white/5 p-3 shadow-inner shadow-white/5">
            <label className="flex min-w-[220px] flex-1 flex-col gap-1 group">
              ...
              <input
                ...
                className="input-cosmic text-sm border-transparent bg-black/20 focus:bg-black/40 focus:border-accent-amethyst hover:bg-black/30 transition-all rounded-md"
              />
            </label>
            <FilterSelect
        ...
        <div className="relative z-10 grid min-h-0 flex-1 gap-4 xl:grid-cols-[1.1fr_0.9fr] px-6 pb-6">
          ...
          <div className="min-h-0 rounded-md bg-black/20 shadow-inner shadow-white/5 flex flex-col">
            {loading ? (
              ...
              <EmptyState
                icon={<Filter size={28} />}
                title={t('experiments.no_matching_runs')}
                description={t('experiments.no_matching_runs_desc')}
                className="experimenthub-empty-state flex-1 min-h-[420px]"
              />
            ) : (
              <div className="h-full overflow-auto p-5 space-y-4 custom-scrollbar">
                <AnimatePresence mode="popLayout">
                  {visibleExperiments.map((exp, idx) => {
                    ...
                    const styles = getThemeColorStyles(theme, isSelected)
                    return (
                      <motion.div
                        layout
                        ...
                        className={`group cursor-pointer rounded-md p-5 transition-all duration-300 ${styles.cardClass}`}
                      >
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div className="min-w-0 flex-1">
                            <div className={`flex items-center gap-2 text-sm ${styles.titleClass}`}>
                              <span className={`truncate ${isSelected ? '' : 'group-hover:text-white'} transition-colors`}>{exp.title}</span>
                              {exp.is_best ? (
                                <span className={`inline-flex shrink-0 items-center gap-1 rounded-md border px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.14em] ${
                                  isSelected
                                    ? (theme === 'light' ? 'border-white/30 bg-white/20 text-white' : 'border-black/20 bg-black/10 text-cyan-955')
                            ...
                            <div className={`mt-2 flex flex-wrap items-center gap-2 text-[11px] ${styles.metaClass}`}>
                              <span className={`inline-flex items-center gap-1.5 rounded-md px-2.5 py-0.5 ${styles.badgeBg} ${styles.badgeText}`}>
                                Task {exp.task_type}
                              </span>
                              <span className={`inline-flex items-center gap-1.5 rounded-md px-2.5 py-0.5 ${styles.badgeBg} ${styles.badgeText}`}>
                                {exp.model_type}
                              ...
                              <span className={getStatusBadgeClass(exp.status, isSelected)}>
                          ...
                          <div className="flex items-center gap-2">
                            <RunChip
                              data-testid={`experiment-compare-toggle-${exp.id}`}
                              selected={selectedCompareIds.includes(exp.id)}
                              onClick={(event) => {
                                event.stopPropagation()
                                handleCompareToggle(exp.id)
                              }}
                              className={
                                selectedCompareIds.includes(exp.id)
                                  ? styles.runChipSelectedClass
                                  : styles.runChipClass
                              }
                            >
                            ...
                            <button
                              ...
                              className={`rounded-md border p-2.5 transition-all hover:scale-105 active:scale-95 ${styles.btnReplay}`}
                            ...
                            <button
                              ...
                              className={`rounded-md border p-2.5 transition-all hover:scale-105 active:scale-95 ${styles.btnPdfView}`}
                            ...
                            <button
                              ...
                              className={`rounded-md border p-2.5 transition-all hover:scale-105 active:scale-95 ${styles.btnPdfDownload}`}
                            ...
                            <button
                              ...
                              className={`rounded-md border p-2.5 transition-all hover:scale-105 active:scale-95 ${styles.btnDelete}`}
                        ...
                        <div className="mt-4 grid gap-3 text-xs md:grid-cols-2 xl:grid-cols-4">
                          <span className="flex items-center gap-2">
                            <span className={`text-[10px] font-bold uppercase tracking-wider ${styles.metricLabelClass}`}>{t('experiments.acc')}</span>
                            <span className={`font-mono px-1.5 py-0.5 rounded-md ${styles.metricValueBg}`}>{(exp.accuracy * 100).toFixed(1)}%</span>
                          </span>
                          <span className="flex items-center gap-2">
                            <span className={`text-[10px] font-bold uppercase tracking-wider ${styles.metricLabelClass}`}>{t('experiments.loss')}</span>
                            <span className={`font-mono px-1.5 py-0.5 rounded-md ${styles.metricValueBg}`}>{exp.loss.toFixed(4)}</span>
                          </span>
                          <span className="flex items-center gap-2">
                            <span className={`text-[10px] font-bold uppercase tracking-wider ${styles.metricLabelClass}`}>{t('experiments.epochs')}</span>
                            <span className={`font-mono px-1.5 py-0.5 rounded-md ${styles.metricValueBg}`}>{exp.epoch_count}</span>
                          </span>
                          <span className="flex items-center gap-2 ml-auto xl:ml-0">
                            <span className={`text-[10px] font-bold uppercase tracking-wider ${styles.metricLabelClass}`}>{t('experiments.created')}</span>
                            <span className={styles.metaClass}>{formatDateTime(exp.created_at)}</span>
                          </span>
                        </div>

                        {exp.notes ? (
                          <div className={`mt-4 rounded-md border px-4 py-3 text-xs leading-relaxed italic ${styles.notesBg}`}>
                            "{exp.notes}"
                          </div>
                        ) : null}
                      </motion.div>
          ...
          {/* ── Right: Detail Panels ── */}
          <div className="min-h-0 overflow-hidden custom-scrollbar rounded-md bg-black/20 shadow-inner shadow-white/5 flex flex-col">
            {/* Right pane tab bar */}
            ...
            <div className="space-y-6">
                ...
                actions={
                  selectedDetail ? (
                    <button
                      ...
                      className={`inline-flex items-center gap-2 rounded-md border px-3 py-2 text-xs font-semibold transition-all hover:scale-[1.02] active:scale-95 ${
                ...
                {detailLoading ? (
                  ...
                  >
                    <div className="grid gap-4 md:grid-cols-2">
                      {/* Context card */}
                      <div className="glass-card rounded-md p-5 text-sm text-starlight border-transparent bg-white/5 hover:bg-white/10 transition-colors shadow-lg">
                        ...
                        <div className="mt-3 space-y-4">
                          <label className="flex flex-col gap-1.5 group">
                            ...
                            <input
                              ...
                              className="input-cosmic text-sm border-transparent bg-black/30 focus:bg-black/50 focus:ring-1 focus:ring-accent-amethyst hover:bg-black/40 transition-all rounded-md"
                            />
                          </label>
                    ...
                    <div className="group">
                      ...
                      <textarea
                        ...
                        className="input-cosmic w-full text-sm leading-relaxed border-transparent bg-white/5 focus:bg-black/30 focus:ring-1 focus:ring-accent-amethyst hover:bg-white/10 transition-all resize-none rounded-md"
                      />
                    </div>
                  ...
                  selectedReport ? (
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-md border border-aurora-green/20 bg-aurora-green/10 px-3 py-2 text-xs font-semibold text-aurora-green hover:bg-aurora-green/20 transition-all"
                      >
              ...
              >
                {detailLoading ? (
                  ...
                  >
                    <div className="grid gap-4 md:grid-cols-2">
                      <div className="glass-card rounded-md p-5 border-transparent bg-white/5 hover:bg-white/10 transition-colors shadow-lg">
                    ...
                    <div className="rounded-md border-transparent bg-amethyst/10 p-5 shadow-[0_4px_20px_rgba(147,51,234,0.15)] ring-1 ring-amethyst/30 hover:bg-amethyst/15 hover:-translate-y-0.5 transition-all">
                    ...
                    {/* Config Snapshot */}
                    <div className="glass-card rounded-md p-5 border-transparent bg-black/40 shadow-inner overflow-hidden">
                      <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-twilight">{t('experiments.config_snapshot')}</div>
                      <pre className="mt-3 overflow-auto rounded-md bg-void/50 p-4 text-[11px] text-starlight/80 font-mono max-h-48 border border-white/5 custom-scrollbar">
              ...
              <Panel
                ...
                actions={
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-md border border-aurora-green/20 bg-aurora-green/10 px-4 py-2 text-xs font-bold text-aurora-green hover:bg-aurora-green/20 transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-30 disabled:hover:scale-100"
                  >
                  ...
                  >
                    <div className="grid gap-3 sm:grid-cols-2">
                      {compareResults.map((item, index) => (
                        <div key={item.experiment.id} className="glass-card rounded-md p-4 text-xs text-starlight border-transparent bg-white/5 hover:bg-white/10 transition-colors shadow-lg">
                    ...
                    {/* Chart */}
                    <div className="h-[300px] w-full rounded-md border border-transparent glass-card p-4 bg-white/5 shadow-lg">
