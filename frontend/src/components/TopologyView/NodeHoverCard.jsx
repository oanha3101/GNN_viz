...
export default function NodeHoverCard() {
  ...
  // Only show while hovering — clicking pins via context menu / K-Hop panel instead
  const activeId = hoveredNodeId

  const summary = useMemo(() => {
    if (activeId === null) return null
    const resolvedGroundTruth = selectedTask === 4 ? communityGroundTruth : groundTruth
    return buildHoverSummary(selectedTask, activeId, snap, graphData, resolvedGroundTruth, lang)
  }, [selectedTask, activeId, snap, graphData, groundTruth, communityGroundTruth, lang])
          ...
          <div className="bg-slate-900/80 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl overflow-hidden shadow-black/50">
            ...
            <div className="bg-white/5 px-4 py-2.5 border-b border-white/5 flex items-center justify-between">
              <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">
                {lang === 'vi' ? 'Thông tin node' : 'Node Inspector'}
              </span>
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="pt-2 mt-2 border-t border-white/5">
                <p className="text-[8px] text-slate-500 italic text-center uppercase tracking-widest font-bold">
                  {lang === 'vi' ? 'Bấm để ghim và xem chi tiết' : 'Click to Pin & View Full Details'}
                </p>
              </div>
            </div>
          </div>
