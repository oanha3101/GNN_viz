...
export default function TaskTopology3({ viewMode = null, reportMode = false }) {
  ...
  }, [snapshots, currentEpochFloat, taskData, focusedEdgeIdx, selectedNodeId, effectiveViewMode])
  ...
  const drawBefore = useCallback((ctx, scale) => {
    ...
    overlayEdgeIndexes.forEach((edgeIdx, order) => {
      ...
      const lineWidth = (isFocused ? 5.6 : isPrimary ? 4.2 : 2.6) / Math.max(scale, 0.75)
      const color = getLinkColor(score).replace(/[\d.]+\)$/, `${alpha})`)
