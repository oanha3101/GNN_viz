...
const TASK_META = {
  1: { nameKey: 'tasks_meta.node_classification', color: '#ef4444', icon: Share2 },
  2: { nameKey: 'tasks_meta.graph_classification', color: '#f97316', icon: Boxes },
  3: { nameKey: 'tasks_meta.link_prediction', color: '#eab308', icon: GitBranch },
  4: { nameKey: 'tasks_meta.community_detection', color: '#22c55e', icon: Network },
  5: { nameKey: 'tasks_meta.graph_embedding', color: '#06b6d4', icon: Orbit },
  6: { nameKey: 'tasks_meta.graph_generation', color: '#8b5cf6', icon: Sparkles },
}
...
function readAccuracy(exp) {
  const value = Number(
    exp?.accuracy
      ?? exp?.metrics_json?.summary?.final_accuracy
      ?? exp?.metrics?.summary?.final_accuracy
      ?? exp?.metrics_json?.summary?.best_score
      ?? exp?.metrics?.summary?.best_score
      ?? 0,
  )
  return Number.isFinite(value) && value > 0 ? value : 0
}

function readLoss(exp) {
  const value = Number(
    exp?.loss
      ?? exp?.metrics_json?.summary?.final_loss
      ?? exp?.metrics?.summary?.final_loss
      ?? 0,
  )
  return Number.isFinite(value) && value > 0 ? value : 0
}
...
export default function DashboardPage() {
  ...
  const projectNameById = useMemo(() => {
    return new Map(projects.map((project) => [project.id, project.title || `#${project.id}`]))
  }, [projects])
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.4fr_1fr]">
        <RecentExperiments
          ...
          onOpen={() => navigate('/app/experiments')}
          t={t}
...
function RecentExperiments({ items, projectNameById, onOpen, t, dateLocale }) {
  return (
    <section className="surface-card p-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <button type="button" onClick={onOpen} className="surface-action self-start sm:self-center">
          {t('dashboard.open_all')} <ArrowRight size={13} />
        </button>
      </div>
      {items.length === 0 ? (
        <div className="empty-state empty-state-compact mt-5">
        ...
        <ul className="mt-4 divide-y divide-line-subtle">
          {items.map((exp) => {
            ...
            const TaskIcon = taskMeta?.icon || Network
            const accuracy = readAccuracy(exp)
            const loss = readLoss(exp)
            const statusLabel = t(`status.${exp.status || 'completed'}`)
            const projectName = projectNameById.get(exp.project_id) || t('dashboard.recent_project_missing')
            const datasetName = exp.dataset_name || t('dashboard.recent_dataset_missing')
            return (
              <li key={exp.id} className="recent-row">
                <div
                  className="recent-row-task"
                  style={{
                    background: `color-mix(in oklab, ${taskColor}, white 80%)`,
                    color: taskColor,
                  }}
                >
                  <TaskIcon size={14} strokeWidth={2.2} />
                </div>
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-2 grid gap-2 text-[11px] text-fg-muted sm:grid-cols-2">
                    <div className="recent-meta-card">
                      <span className="recent-meta-label">{t('dashboard.recent_project_label')}</span>
                      <span className="truncate font-semibold text-fg">{projectName}</span>
                    </div>
                    <div className="recent-meta-card">
                      <span className="recent-meta-label">{t('dashboard.recent_dataset_label')}</span>
                      <span className="truncate font-semibold text-fg">{datasetName}</span>
                    </div>
                  </div>
                </div>
                <div className="hidden min-w-[120px] flex-col items-end gap-1 sm:flex">
                  ...
                  <div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-fg-faint">
                    {t('dashboard.recent_accuracy_label')}
                  </div>
                  ...
                  <div className="text-[11px] text-fg-muted">
                    {loss ? `${t('dashboard.recent_loss_label')}: ${loss.toFixed(3)}` : ''}
                  </div>
                </div>
