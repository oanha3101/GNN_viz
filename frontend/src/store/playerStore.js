...
const usePlayerStore = create((set, get) => ({
  ...
  loadSnapshots: (snapshots) => {
    const configuredEpochs = snapshots.reduce((target, snapshot) => {
      const explicit = Number(snapshot?.epochs_target ?? snapshot?.model_hyperparams?.epochs_target)
      return Number.isFinite(explicit) ? Math.max(target, explicit) : target
    }, snapshots.length)
    set((s) => ({
      snapshots,
      totalEpochs: snapshots.length,
      configuredEpochs,
      // Preserve current position if already viewing; otherwise stay at the start
      currentEpoch: snapshots.length > 0 ? Math.min(Math.max(0, s.currentEpoch), snapshots.length - 1) : 0,
      currentEpochFloat: snapshots.length > 0 ? Math.min(Math.max(0, s.currentEpochFloat), snapshots.length - 1) : 0,
  ...
  addSnapshot: (snapshot) => {
    set((s) => {
      ...
      const explicitTarget = Number(snapshot?.epochs_target ?? snapshot?.model_hyperparams?.epochs_target)
      const update = {
        snapshots: [...s.snapshots, snapshot],
        totalEpochs: newLen,
        configuredEpochs: Number.isFinite(explicitTarget)
          ? Math.max(s.configuredEpochs || 0, explicitTarget)
          : Math.max(s.configuredEpochs || 0, newLen),
      }
